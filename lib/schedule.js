// server/schedule.js
const { readSheetObjects } = require("./sheets");

const DOW = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"];
const toYMD = (d) => d.toISOString().slice(0, 10);
const addDays = (d, n) => {
  const x = new Date(d);
  x.setDate(x.getDate() + n);
  return x;
};
const yyyymm = (s) => String(s).slice(0, 7); // "YYYY-MM" from "YYYY-MM-DD"

// 고정 공휴일(필요시 더 추가)
function fixedHolidayName(d) {
  const map = {
    "01-01": "신정",
    "03-01": "3·1절",
    "05-05": "어린이날",
    "06-06": "현충일",
    "08-15": "광복절",
    "10-03": "개천절",
    "10-09": "한글날",
    "12-25": "성탄절",
  };
  return map[toYMD(d).slice(5)] || null;
}

// ⬇️ (자동 모드에서만 사용) 달별 '월요일' 중 마지막/마지막-2를 시험일로 선택
function computeTestMondays(calendar) {
  // calendar: [{date, weekday, skipReason?}]
  const byMonth = new Map();
  for (const day of calendar) {
    if (day.weekday !== "MON") continue;
    const key = day.date.slice(0, 7);
    if (!byMonth.has(key)) byMonth.set(key, []);
    byMonth.get(key).push(day.date);
  }
  const testSet = new Set();
  for (const [mm, dates] of byMonth.entries()) {
    dates.sort(); // 오름차순
    const n = dates.length; // 그 달 월요일 개수
    if (n >= 1) testSet.add(dates[n - 1]); // 마지막 주 월요일
    if (n >= 3) testSet.add(dates[n - 3]); // 마지막 전전 주 월요일(4개면 2번째, 5개면 3번째)
  }
  return testSet;
}

// lanes 입력을 실제 유닛 큐로 변환
function buildQueueFactory(mainBook, vocaBook) {
  return function buildQueue(list, isVocab = false) {
    const getAll = (materialId) =>
      (isVocab ? vocaBook : mainBook)
        .filter((u) => String(u.material_id) === String(materialId))
        .sort((a, b) => Number(a.order || 0) - Number(b.order || 0));
    const out = [];
    list.forEach((book, i) => {
      const all = getAll(book.materialId);
      if (!all.length) return;
      let startIdx = 0;
      if (i === 0 && book.startUnitCode) {
        const j = all.findIndex(
          (u) => String(u.unit_code) === String(book.startUnitCode)
        );
        startIdx = j >= 0 ? j : 0;
      }
      out.push(...all.slice(startIdx));
    });
    return out;
  };
}

// ✅ 메인 함수: monthly test 반영
async function generatePlan(body) {
  const {
    studentName,
    startDate,
    endDate,
    days,
    lanes = {},
    userSkips = [],
    // ⬇️ 새 파라미터
    tests = [], // [{date:"YYYY-MM-DD", title:"Monthly Test 1", materialId?, notes?}, ...]
    testMode = "explicit", // "explicit" | "auto"  (tests가 없거나 자동을 원할 때 "auto")
  } = body;

  if (!startDate) throw new Error("startDate 누락");
  if (!endDate) throw new Error("endDate 누락");
  if (!days) throw new Error("days 누락");

  const want = new Set(
    String(days)
      .split(",")
      .map((s) => s.trim().toUpperCase())
  );

  // 사용자 지정 스킵 맵 (date -> label)
  const userSkipByDate = new Map(
    (userSkips || []).map((x) => {
      const d = String(x.date || "").slice(0, 10);
      const label =
        x.type === "vacation"
          ? "휴가"
          : x.type === "sick"
          ? "질병"
          : x.type === "other"
          ? x.reason?.trim()
            ? `기타: ${x.reason.trim()}`
            : "기타"
          : x.reason?.trim() || "기타";
      return [d, label];
    })
  );

  // 시트 로드 (시트 이름/범위는 프로젝트에 맞게 조정)
  const [mainBook, vocaBook, holidays] = await Promise.all([
    readSheetObjects("mainBook"),
    readSheetObjects("vocaBook").catch(() => []),
    readSheetObjects("Holidays").catch(() => []),
  ]);

  const holidayNameByDate = new Map(
    holidays.map((h) => {
      const date = String(h.date || "").slice(0, 10);
      const name = String(h.name || h.title || "공휴일").trim() || "공휴일";
      return [date, name];
    })
  );

  const begin = new Date(startDate);
  const end = new Date(endDate);

  // ⬇️ 기간 내 수업일 캘린더 구성(+스킵/공휴일 마킹)
  const calendar = [];
  for (let d = new Date(begin); d <= end; d = addDays(d, 1)) {
    const wd = DOW[d.getDay()];
    const y = toYMD(d);
    if (!want.has(wd)) continue;

    const fixed = fixedHolidayName(d);
    const named = holidayNameByDate.get(y);
    const user = userSkipByDate.get(y);
    const skipReason = user || named || (fixed ? `공휴일: ${fixed}` : null);

    calendar.push({ date: y, weekday: wd, skipReason });
  }

  // ⬇️ 시험일 수집
  let testIndex = new Map(); // date -> [{title, materialId, notes}]
  if (testMode === "explicit" && Array.isArray(tests) && tests.length) {
    // 명시적 입력 모드
    for (const t of tests) {
      const d = String(t.date || "").slice(0, 10);
      if (!d) continue;
      if (!testIndex.has(d)) testIndex.set(d, []);
      testIndex.get(d).push({
        title: t.title || "Monthly Test",
        materialId: t.materialId || "",
        notes: t.notes || "",
      });
    }
  } else {
    // 자동 모드: 마지막·마지막-2 번째 월요일을 시험으로 지정
    const testSet = computeTestMondays(calendar);
    for (const d of testSet) {
      testIndex.set(d, [{ title: "Monthly Test", materialId: "", notes: "" }]);
    }
  }

  // ⬇️ 시험일이 스킵/공휴일과 충돌하면 제외 (정책: 시험은 수업 있는 날에만 치름)
  for (const [d, _items] of [...testIndex.entries()]) {
    const day = calendar.find((c) => c.date === d);
    if (!day || day.skipReason) {
      testIndex.delete(d);
    }
  }

  // 단원 큐 준비
  const makeQueue = buildQueueFactory(mainBook, vocaBook);
  const laneDef = {
    main1: makeQueue(lanes.main1 || [], false),
    main2: makeQueue(lanes.main2 || [], false),
    vocab: makeQueue(lanes.vocab || [], true),
  };

  // 배정: 시험 우선 → 없으면 main1/main2/vocab
  const idx = { main1: 0, main2: 0, vocab: 0 };
  const out = [];

  for (const day of calendar) {
    // 스킵일은 그대로 출력
    if (day.skipReason) {
      out.push({
        date: day.date,
        weekday: day.weekday,
        source: "skip",
        reason: day.skipReason,
      });
      continue;
    }

    // 1) 시험 우선
    const todaysTests = testIndex.get(day.date) || [];
    if (todaysTests.length) {
      for (const t of todaysTests) {
        out.push({
          date: day.date,
          weekday: day.weekday,
          source: "test",
          title: t.title || "Monthly Test",
          material_id: t.materialId || "",
          unit_code: "",
          lecture_range: "",
          pages: "",
          wb: "",
          dt_vocab: "",
          key_sents: t.notes || "",
        });
      }
      continue; // 시험 있으면 그 날은 시험만(정책)
    }

    // 2) 메인/보카
    for (const lane of ["main1", "main2", "vocab"]) {
      const q = laneDef[lane];
      const k = idx[lane];
      const u = q[k];
      if (!u) continue;
      idx[lane] = k + 1;

      if (lane === "vocab") {
        out.push({
          date: day.date,
          weekday: day.weekday,
          source: "vocab",
          material_id: u.material_id,
          unit_code: u.unit_code,
          lecture_range: u.lecture_range || u.lecture || "",
          pages: "",
          wb: "",
          dt_vocab: "",
          key_sents: "",
          vocab_session: u.vocab_session || "",
          vocab_range: u.vocab_range || "",
        });
      } else {
        out.push({
          date: day.date,
          weekday: day.weekday,
          source: "main",
          material_id: u.material_id,
          unit_code: u.unit_code,
          lecture_range: u.lecture_range || u.lecture || u.title || "",
          pages: u.pages || "",
          wb: u.wb || "",
          dt_vocab: u.dt_vocab || "",
          key_sents: u.key_sents || "",
        });
      }
    }
  }

  // 정렬: 날짜 → source 우선순위(test > main > vocab > skip)
  const rank = { test: 0, main: 1, vocab: 2, skip: 9 };
  return out.sort((a, b) => {
    if (a.date !== b.date) return a.date < b.date ? -1 : 1;
    return (rank[a.source] ?? 9) - (rank[b.source] ?? 9);
  });
}

module.exports = { generatePlan };
