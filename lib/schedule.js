// /lib/schedule.js — 전체 교체 (시간대 문제 최종 해결)

import { readSheetObjects } from "./sheets.js";

const DOW = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"];

// [수정] new Date()의 시간대 문제를 원천적으로 해결하는 함수들
  ...
  return function buildQueue(list, isVocab = false) {
    ...
    (list || []).forEach((book) => {
      ...
      if (book.endUnitCode) {
        ...
        // +1을 하여 endIdx가 slice에서 포함되도록 함
        endIdx = j >= 0 ? j + 1 : all.length;
      }

      // 시작 인덱스가 종료 인덱스보다 크면 빈 배열을 추가
      if (startIdx >= endIdx) {
        out.push([]);
      } else {
        out.push(...all.slice(startIdx, endIdx));
      }
    });
    return out;
  };
...
export async function generatePlan(body) {
  ...
  for (const day of calendar) {
    ...
    for (const lane of ["main1", "main2", "vocab"]) {
      ...
      // [추가] order 값을 확인하여 OT 여부 판단
      const isOT = String(u.order || "").trim() === "1";
