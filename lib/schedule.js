...
function buildQueueFactory(mainBook, vocaBook) {
  return function buildQueue(list, isVocab = false) {
    ...
    (list || []).forEach((book) => {
      ...
      const firstUnit = slicedUnits[0];
      const isReturning = seenMaterialIds.has(book.materialId);

      // [핵심 수정] OT 또는 '복귀' 상태 결정
      if (!isVocab) {
        if (String(firstUnit.order || "").trim() === "1") {
          firstUnit.isOT = true;
        } else if (isReturning) {
          // '복귀' 마커 객체를 진도 목록 가장 앞에 추가
          const returnMarker = {
            isReturn: true,
      ...
      seenMaterialIds.add(book.materialId);
    });
    return out;
  };
}
